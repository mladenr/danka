...
def process_bill(bill, payments, tpd):
    first_not_processed_payment = findFirst(lambda payment: not payment['processed'], payments)
    if first_not_processed_payment is not None:
        ...
        if bill['billDebt'] > 0:
            tpd = process_bill(bill, payments, tpd)
...
print_totals('ALL', bill_list, payment_list)
...
for project_id, project_bills in GroupUtils.group_bill_by_project(bill_list).items():
    ...
    print_totals(project_id, project_bills, project_payments)

    for bill2 in project_bills:
        print('   ----------------------------')
        print('   Processing bill: ' + str(bill2['amount']))

        bill2['billDebt'] = bill2['amount']
        total_project_debt = total_project_debt + bill2['amount']

        print('   Total project debt before payment processing: ' + str(total_project_debt))
        print('   Total bill debt before payment processing: ' + str(bill2['billDebt']))

        total_project_debt = process_bill(bill2, project_payments, total_project_debt)

        print('   Total bill debt after payments processing: ' + str(bill2['billDebt']))
        print('   Bill isPaidOff: ' + str(bill2['isPaidOff']))
        print('   ----------------------------')

print('Total project debt: ' + str(total_project_debt))
print_results(bill_list, payment_list)
global data_frame
data_frame = pd.DataFrame(bill_list)
print(data_frame)
